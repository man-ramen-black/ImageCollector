package com.kakakobank.imagecollector.model.network

import com.jakewharton.retrofit2.converter.kotlinx.serialization.asConverterFactory
import com.kakakobank.imagecollector.model.data.NetworkResult
import com.kakakobank.imagecollector.model.data.ResponseParsingException
import com.kakakobank.imagecollector.model.data.UnknownNetworkException
import com.kakakobank.imagecollector.util.KBJson
import com.kakakobank.imagecollector.util.Log
import com.kakakobank.imagecollector.util.Util.ifThen
            ...
            val retrofit = Retrofit.Builder()
                .addConverterFactory(
                    JsonConverterFactory(
                        GsonConverterFactory.create(KBJson.gson),
                        KBJson.kotlinSerialization.asConverterFactory("application/json".toMediaType())
                    )
                )
    ...
    private fun onFailed(t: Throwable) : NetworkResult<T> {
        ...
        return try {
            ...
            if (KotlinSerialization.isKotlinSerialization(annotations)) {
                KBJson.tryForKS { throw t }
            } else {
                KBJson.tryForGson { throw t }
            }
    ...
    private fun parse(response: String?, type: Type) : T? {
        ...
        return when {
            annotations.any { it.annotationClass == KotlinSerialization::class } -> {
                KBJson.from(response, type, true)
            }
            else -> {
                ...
                KBJson.from(response, type, false)
            }
        }
    }
